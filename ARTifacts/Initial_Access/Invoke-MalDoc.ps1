...
function Invoke-MalDoc($macro_choice, $office_version, $office_product) {
    ...
    if ($macro_choice -eq "1"){
        $macro_code = "Sub Test()`n Shell `"cmd.exe /c calc.exe`" `nEnd Sub" 
    } 
    ...
    elseif ($macro_choice -eq "4"){
        $macro_code = "Sub Test()`n Shell `"Schtasks.exe /create /tn `"`"calculator`"`" /tr c:\Windows\System32\Calc /sc once /st 00:00`" `nEnd Sub"  
    } 
